<script>
  import * as api from "../../helpers/api";
  import userStore from "../../stores/userStore";
  import { onMount, onDestroy } from "svelte";
  import LoadingSpinner from "../../components/Ui/LoadingSpinner";
  import ls from "local-storage";
  import Message from "../../components/Message";

  let unsubscribe;
  let currentUser = null;
  let adminSettings = [];
  let settings = [];
  let userId;
  let message;
  let messageType = "warning";
  let userOption;
  let newUser;
  let newQuote;
  let isLoading = false;

  const token = ls.get("jwt");

  async function updateSettings(e) {
    isLoading = true;

    if (e.target.value === "user") {
      newUser = !newUser;
    }

    if (e.target.value === "quote") {
      newQuote = !newQuote;
    }

    const userObject = {
      newUser: newUser,
      newQuote: newQuote
    };

    try {
      const res = await api.patch( `admin/settings/${userId}`, userObject, token );

      if (res && res.message) {
        isLoading = false;
        return (message = res.message);
      } else {
        isLoading = false;
        return userStore.setUser(res.user);
      }
    } catch (err) {
      return message = err;
    }
  }

  onMount(() => {
    unsubscribe = userStore.subscribe(userData => {
      currentUser = userData;
      if (currentUser) {
        userId = currentUser._id;
        newQuote = currentUser.settings.newQuote;
        newUser = currentUser.settings.newUser;
      }
    });
  });

  onDestroy(() => {
    if (unsubscribe) {
      unsubscribe();
    }
  });

  function closeMessage() {
    message = null;
  }
  
</script>

<div class="card">
  <header class="card-header">
    <div class="card-header-title">&nbsp;</div>
    <div class="card-header-icon">:::</div>
  </header>
  <div class="card-content">
    {#if isLoading}
      <LoadingSpinner />
    {/if}
    {#if message}
      <Message
        {message}
        messageType="warning"
        on:closeMessageEvent={closeMessage} />
    {/if}
    <form class="form">
      <div class="no-margin">
        <label class="label">Email Notifications:</label>
        <p class="help">
          An email will be sent to notify you when any of the slections below
          are checked.
        </p>
      </div>
      <div class="inputGroup">
        <input
          id="option1"
          type="checkbox"
          value={'user'}
          on:change={updateSettings}
          checked={newUser} />
        <label for="option1">New User</label>
      </div>
      <div class="inputGroup">
        <input
          id="option2"
          type="checkbox"
          value={'quote'}
          on:change={updateSettings}
          checked={newQuote} />
        <label for="option2">New Quote</label>
      </div>
    </form>
  </div>
</div>

<style>
  .no-margin label {
    margin-bottom: 0;
    line-height: 1;
  }
  .no-margin .help {
    margin-top: 0;
  }
  .inputGroup {
    background-color: #fff;
    display: block;
    margin: 10px 0;
    position: relative;
  }
  .inputGroup label {
    padding: 12px 30px;
    width: 100%;
    display: block;
    text-align: left;
    color: #3c454c;
    cursor: pointer;
    position: relative;
    z-index: 2;
    transition: color 200ms ease-in;
    overflow: hidden;
  }
  .inputGroup label:before {
    width: 10px;
    height: 10px;
    border-radius: 50%;
    content: "";
    background-color: #5562eb;
    position: absolute;
    left: 50%;
    top: 50%;
    transform: translate(-50%, -50%) scale3d(1, 1, 1);
    transition: all 300ms cubic-bezier(0.4, 0, 0.2, 1);
    opacity: 0;
    z-index: -1;
  }
  .inputGroup label:after {
    width: 32px;
    height: 32px;
    content: "";
    border: 2px solid #d1d7dc;
    background-color: #fff;
    background-image: url("data:image/svg+xml,%3Csvg width='32' height='32' viewBox='0 0 32 32' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M5.414 11L4 12.414l5.414 5.414L20.828 6.414 19.414 5l-10 10z' fill='%23fff' fill-rule='nonzero'/%3E%3C/svg%3E ");
    background-repeat: no-repeat;
    background-position: 2px 3px;
    border-radius: 50%;
    z-index: 2;
    position: absolute;
    right: 30px;
    top: 50%;
    transform: translateY(-50%);
    cursor: pointer;
    transition: all 200ms ease-in;
  }
  .inputGroup input:checked ~ label:before {
    background: #f9f9f9;
    transform: translate(-50%, -50%) scale3d(56, 56, 1);
    opacity: 1;
  }
  .inputGroup input:checked ~ label:after {
    background-color: #0f8892;
    border-color: #0f8892;
  }
  .inputGroup input {
    width: 32px;
    height: 32px;
    order: 1;
    z-index: 2;
    position: absolute;
    right: 30px;
    top: 50%;
    transform: translateY(-50%);
    cursor: pointer;
    visibility: hidden;
  }
  .form {
    padding: 0 16px;
    max-width: 550px;
    margin: 50px auto;
    font-size: 18px;
    font-weight: 600;
    line-height: 36px;
  }
</style>
